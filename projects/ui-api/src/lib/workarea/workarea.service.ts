...
import { WorkAreaDataParams } from '../classes/workareadataparams';
import { SearchParams } from '../classes/search-params';
import { WorkAreaDataRequestModel } from '../classes/workareadataparams';
...
export class WorkareaService implements IGridInterface {
  ...
  getData(params: WorkAreaDataParams): Observable<SearchParams> {
    ...
    const dataRequest: WorkAreaDataRequestModel = new WorkAreaDataRequestModel();
    ...
    return this.getWorkAreaItems(this.currentWorkArea).pipe(switchMap(resp => {
      return this.http.post(`${Global.API_URL}/api/workarea/data/`, dataRequest).pipe(map(res => {
        const result = res as SearchParams;
  ...
  getWorkAreaItems(workArea: WorkAreaModel): Observable<WorkAreaItemsList> {
    ...
    return this.getLocalWorkAreaItems(workArea).pipe(switchMap(workAreaItemList => {
      ...
      if (!workAreaItemList.pathExists) {
        const msg = this.locale.resourceStrings['WORK_AREA_ACCESS'];
        const title = this.locale.resourceStrings['ERROR'];
      ...
      return this.getExtractionItems(workAreaItemList).pipe(switchMap(extractionItems => {
        ...
        return this.getPluginDefinitions().pipe(switchMap(plugins => {
          ...
          const incomingWillExtract = new IncomingWillExtract();
  ...
  updateValidation(workArea: WorkAreaModel, workAreaItemList: WorkAreaItemsList): WorkAreaModel {
    // Update Validation.
    const ValidationState_Valid = 1;
    const ValidationState_Invalid = 2;
  ...
  getExtractionItems(workAreaItems: WorkAreaItemsList): Observable<WorkAreaExtractionItems> {
    ...
    return this.http.put(`${Global.API_URL}/api/workarea/select/${workAreaItems.WorkAreaId}`, workAreaItems).pipe(map(resp => resp as WorkAreaExtractionItems), map(extractionItems => {
      if (extractionItems != null) {
        const invalidItemsArray = extractionItems.invalidItems;
  ...
  extractAndUpdateItems(items: WorkAreaDBRecordModel[], path: string): Observable<any> {
    ...
    const progressValue = 100 / items.length;
  ...
  setIcon(workAreaModel: WorkAreaModel) {
    ...
    const ValidationState_Unchecked = 0;
    const ValidationState_Valid = 1;
    const ValidationState_Invalid = 2;
    const DriveType_Local = 'L';
    const DriveType_Network = 'N';
