...
export class ColumnsService {
  ...
  getColumns(): Observable<Array<FieldDefinition>> {
    const obs = this.http.get(`${Global.API_URL}/api/column/allavailable`).map(data => this.columns = data as Array<FieldDefinition>);
    return obs;
  }

  getColumnSets(): Observable<Array<ColumnSet>> {
    const obs = this.http.get(`${Global.API_URL}/api/columnset`).map(data => this.columnSets = data as Array<ColumnSet>);
    return obs;
  }

  getColumnSetColumns(columnSetId: string): Observable<Array<Column>> {
    let setColumns: Array<Column>;
    const obs = this.http.get(`${Global.API_URL}/api/column/${columnSetId}`).map(data => setColumns = data as Array<Column>);
    return obs;
  }
  ...
  getGridColumns(): Observable<Array<FieldDefinition>> {
    ...
    return this.getColumnSets().pipe(
      mergeMap(data => this.getColumnSetColumns(data.filter(d => d.name === "SearchResultColumns")[0].id).map(
